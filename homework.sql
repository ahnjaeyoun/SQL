...
-- 누구인가? (국문학과의 '학과코드'는 학과 테이블을 조회해서 찾아내도록 하자)
...
-- 1. 영어영문학과(학과코드 002) 학생들의 학번과 이름, 입학년도를 입학년도가 빠른
...
-- 3. 춘 기술대학교의 남자 교술들의 이름과 나이를 출력하는 SQL 문장을 작성하시오. 단
...
-- "이름" 이 찍히도록 한다. (성이 2 자인 경우의 교수는 없다고 가정하시오)
...
SELECT STUDENT_NAME 
FROM TB_STUDENT
WHERE SUBSTR(EXTRACT(YEAR FROM ENTRANCE_DATE), 1, 2) = 20;

SELECT STUDENT_NO, STUDENT_NAME 
FROM TB_STUDENT
WHERE ABS(SUBSTR(EXTRACT(YEAR FROM ENTRANCE_DATE), 3, 4) - SUBSTR(STUDENT_SSN, 1, 2)) = 19
ORDER BY STUDENT_NAME;
-- 답안작성
...
-- 7. 
 ...
 SELECT TO_CHAR(TO_DATE('99/10/11'), 'YYYY"년" MM"월" DD"일"') 년월일 FROM DUAL; 
 SELECT TO_CHAR(TO_DATE('49/10/11'), 'YYYY"년" MM"월" DD"일"') 년월일 FROM DUAL; 
 SELECT TO_CHAR(TO_DATE('99/10/11'), 'RRRR"년" MM"월" DD"일"') 년월일 FROM DUAL; 
 SELECT TO_CHAR(TO_DATE('49/10/11'), 'RRRR"년" MM"월" DD"일"') 년월일 FROM DUAL;
