...
SELECT ROUND(123.456, 0) FROM DUAL; -- 123, 소수점 첫 번째 자리에서 반올림
...
-- SYSDATE : 시스템에 현재 시간 (년, 월, 일, 시, 분, 초)을 반한
...
ABS(CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE))) "근무한 개월수 ",
...
-- ""를 쓰지 않으면 년, 월, 일이 날짜를 나타내는 패턴으로 인식이 안되서 오류 발생
--> ""쌍따옴표를 이용해서 단순한 무자로 인식시키면 해결됨
...
-- TO_NUMBER(문자데이터, [VHAOT]) : 문자형데이터 숫자 데이터로 변경
...
-- EMLOYEE 테이블에서 보너스를 받으면 '0', 안받으면 'X' 조회
...
-- 비교하조가 하는 값 또는 컬럼이 조건식과 같으면 결과값 반환
...
-- 급여 최소값, 가장 빠른 입사일, 알파뱃 순서가 가장 빠른 이메일
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장 늦은 입사일, 알파뱃 순서가 가장 느린 이메일
