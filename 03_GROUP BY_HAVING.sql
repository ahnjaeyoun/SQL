...
-- GROUP BY절 : 같은 값들이 여러ㅐ 기록된 컬럼을 가지고 같은 값들을 하나의 그룹으로 묶음
--	같은 값들을 하나의 그룹으로 묶음
-- GROUP BY 컬럼명 | 함수식, ...
...
SELECT DEPT_CODE, SUM(SALARY) -- 5번
FROM EMPLOYEE -- 1번
GROUP BY DEPT_CODE; -- 3번 --> DEPT_CODE가 같은 행끼리 하나의 그룹이 됨
-- GROUP BY를 쓰지 않으면
-- SQL Error [937] [42000]: ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

-- EMPLOYEE 테이블에서
-- 직급코드가 같은 사람의 지급코드, 급여 평균, 인원수를
-- 직급코드 오름 차순으로 조회
